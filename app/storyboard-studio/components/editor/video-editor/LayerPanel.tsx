...
export function LayerPanel({
  ...
  const btnCls = "w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 transition text-[#A0A0A0] hover:text-white";
  const inputCls = "w-full px-1.5 py-1 text-[10px] bg-[#1a1a24] border border-[#2a2a35] rounded text-white outline-none";
  const labelCls = "text-[9px] text-[#6E6E6E] uppercase tracking-wide";
  ...
  return (
    <div className="w-52 bg-[#0f0f17] border-l border-[#1e1e28] flex flex-col overflow-hidden shrink-0">
      {/* Header */}
      <div className="px-2 py-1.5 border-b border-[#1e1e28] flex items-center gap-1.5">
        <Layers className="w-3.5 h-3.5 text-teal-400" />
        <span className="text-[11px] font-semibold text-white">Layers</span>
      </div>

      {/* Tool buttons */}
      <div className="px-1.5 py-1.5 border-b border-[#1e1e28]">
        <div className="flex flex-wrap gap-0.5">
          ...
          <div className="flex items-center gap-1 px-1 rounded bg-[#1a1a24] border border-[#2a2a35]" title="Background">
            <span className="text-[8px] text-[#6E6E6E]">BG</span>
            <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} className="w-4 h-4 rounded cursor-pointer border-none" />
          </div>
        </div>

        {/* Media picker with thumbnails */}
        {mediaItems.length > 0 && (
          <div className="relative mt-1">
            <button onClick={() => setShowMediaPicker(!showMediaPicker)}
              className="w-full px-1.5 py-1 text-[10px] bg-[#1a1a24] border border-[#2a2a35] rounded text-violet-400 hover:bg-[#1e1e2e] transition text-left">
              {loadingMedia ? "Loading..." : "+ Image / Video overlay"}
            </button>
            {showMediaPicker && (
              <div className="absolute left-0 right-0 top-full mt-1 bg-[#1a1a24] border border-[#2a2a35] rounded-lg shadow-2xl z-50 max-h-48 overflow-y-auto">
                {mediaItems.map((f: any) => {
                  ...
                  return (
                    <button key={f._id} onClick={async () => {
                      ...
                      // Fetch as blob to bypass CORS for video/image overlays
                    ...
                    }} className="w-full flex items-center gap-2 px-2 py-1.5 hover:bg-white/5 transition text-left">
                      {f.fileType === "image" ? (
                        <img src={origSrc} className="w-8 h-5 rounded object-cover shrink-0 bg-black" alt="" />
                      ) : f.fileType === "video" ? (
                        <div className="w-8 h-5 rounded bg-[#0f0f17] flex items-center justify-center shrink-0 border border-[#2a2a35]">
                      ...
                      <span className="text-[9px] text-[#c0c0c0] truncate flex-1">{(f.filename || f.model || "Untitled").slice(0, 22)}</span>
                      <span className="text-[7px] text-[#4A4A4A] uppercase shrink-0">{f.fileType}</span>
                    </button>
                  );
                })}
              </div>
      ...
      {/* Layer list — lock, eye, type, label, reorder */}
      <div className="flex-1 overflow-y-auto px-1 py-1 space-y-0.5 min-h-0">
        {overlayLayers.length === 0 ? (
          <div className="text-center py-6 text-[10px] text-[#3D3D3D]">No layers yet</div>
        ) : (
          [...overlayLayers].reverse().map(layer => {
            ...
            return (
              <div key={layer.id}
                className={`flex items-center gap-0.5 px-1 py-1 rounded text-[10px] cursor-pointer transition ${
                  isSel ? "bg-teal-500/15 text-teal-300 ring-1 ring-teal-500/30" : "text-[#A0A0A0] hover:bg-white/5"
                }`}
                onClick={() => setSelectedOverlayId(layer.id)}>
                {/* Lock */}
                <button onClick={(e) => { e.stopPropagation(); patch(layer.id, { locked: !layer.locked }); }}
                  className="shrink-0 p-0.5 hover:text-white transition" title={layer.locked ? "Unlock" : "Lock"}>
                  {layer.locked ? <Lock className="w-2.5 h-2.5 text-red-400/70" /> : <Unlock className="w-2.5 h-2.5 text-[#3D3D3D]" />}
                </button>
                {/* Visibility */}
                <button onClick={(e) => { e.stopPropagation(); patch(layer.id, { visible: !(layer.visible ?? true) }); }}
                  ...
                  {(layer.visible ?? true) ? <Eye className="w-2.5 h-2.5" /> : <EyeOff className="w-2.5 h-2.5 text-[#3D3D3D]" />}
                </button>
                {/* Type icon */}
                ...
                <div className="flex shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); moveLayer(layer.id, 1); }} className="p-0.5 hover:text-white"><ChevronUp className="w-2.5 h-2.5" /></button>
                  <button onClick={(e) => { e.stopPropagation(); moveLayer(layer.id, -1); }} className="p-0.5 hover:text-white"><ChevronDown className="w-2.5 h-2.5" /></button>
                </div>
              </div>
            );
          })
      ...
      {/* Properties panel */}
      {sel && (
        <div className="border-t border-[#1e1e28] px-2 py-2 space-y-2 overflow-y-auto max-h-[45%]">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-semibold text-white">Properties</span>
            <div className="flex gap-1">
              <button onClick={() => duplicate(sel)} className="p-1 text-[#6E6E6E] hover:text-teal-400 transition" title="Duplicate"><Copy className="w-3 h-3" /></button>
              <button onClick={() => { onBeforeChange?.(); setOverlayLayers(p => p.filter(l => l.id !== sel.id)); setSelectedOverlayId(null); }}
                className="p-1 text-[#6E6E6E] hover:text-red-400 transition" title="Delete"><Trash2 className="w-3 h-3" /></button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1 mt-0.5">
              ...
              <span className="text-[9px] text-[#4A4A4A] self-center">→</span>
          ...
          <div>
            ...
            <div className="flex gap-1 text-[8px] text-[#4A4A4A] mt-0.5"><span className="flex-1 text-center">X</span><span className="flex-1 text-center">Y</span><span className="flex-1 text-center">W</span><span className="flex-1 text-center">H</span></div>
          </div>
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              ...
              <input type="number" min={-360} max={360} value={sel.rotation || 0} onChange={(e) => patch(sel.id, { rotation: parseInt(e.target.value) || 0 })} className={`${inputCls} mt-0.5`} />
            </div>
            <div className="flex-1">
              ...
              <input type="range" min={0} max={100} value={sel.opacity ?? 100} onChange={(e) => patch(sel.id, { opacity: parseInt(e.target.value) })}
                className="w-full h-1 accent-teal-500 cursor-pointer mt-2" />
            </div>
          </div>
          ...
          {sel.type === "text" && (
            <div className="space-y-1.5">
              <div><span className={labelCls}>Text</span><input value={sel.text || ""} onChange={(e) => patch(sel.id, { text: e.target.value })} className={`${inputCls} mt-0.5`} /></div>
              <div className="flex gap-1">
                ...
                <input type="number" min={12} max={200} value={sel.fontSize || 48} onChange={(e) => patch(sel.id, { fontSize: parseInt(e.target.value) || 48 })} className={`${inputCls} w-12`} />
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Text</span><input type="color" value={sel.fontColor || "#FFFFFF"} onChange={(e) => patch(sel.id, { fontColor: e.target.value })} className="w-5 h-5 rounded cursor-pointer border-none" /></div>
                <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">BG</span><input type="color" value={sel.backgroundColor === "transparent" ? "#000000" : (sel.backgroundColor || "#000000")} onChange={(e) => patch(sel.id, { backgroundColor: e.target.value })} className="w-5 h-5 rounded cursor-pointer border-none" /></div>
                <button onClick={() => patch(sel.id, { backgroundColor: "transparent" })}
                  className={`px-1.5 py-0.5 text-[8px] rounded border border-[#2a2a35] ${sel.backgroundColor === "transparent" || !sel.backgroundColor ? "bg-teal-500/20 text-teal-300" : "text-[#6E6E6E]"}`}>None</button>
              </div>
              <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Radius</span><input type="number" min={0} max={50} value={sel.borderRadius || 0} onChange={(e) => patch(sel.id, { borderRadius: parseInt(e.target.value) || 0 })} className={`${inputCls} w-12`} /></div>
            </div>
          )}
          ...
          {sel.type === "shape" && (
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                <span className="text-[8px] text-[#6E6E6E]">Stroke</span>
                ...
                <input type="number" min={1} max={20} value={sel.strokeWidth || 4} onChange={(e) => patch(sel.id, { strokeWidth: parseInt(e.target.value) || 4 })} className={`${inputCls} w-10`} />
              </div>
              {(sel.shapeType === "rectangle" || sel.shapeType === "circle") && (
                <div className="flex items-center gap-1.5">
                  <span className="text-[8px] text-[#6E6E6E]">Fill</span>
                  ...
                  <button onClick={() => patch(sel.id, { fillColor: "transparent" })}
                    className={`px-1.5 py-0.5 text-[8px] rounded border border-[#2a2a35] ${!sel.fillColor || sel.fillColor === "transparent" ? "bg-teal-500/20 text-teal-300" : "text-[#6E6E6E]"}`}>None</button>
                </div>
              )}
              {sel.shapeType === "rectangle" && (
                <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Radius</span><input type="number" min={0} max={100} value={sel.borderRadius || 0} onChange={(e) => patch(sel.id, { borderRadius: parseInt(e.target.value) || 0 })} className={`${inputCls} w-12`} /></div>
              )}
            </div>
          )}
          ...
          {(sel.type === "video" || sel.type === "image") && (
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                <span className="text-[8px] text-[#6E6E6E]">Border</span>
                ...
                <input type="number" min={0} max={20} value={sel.borderWidth || 0} onChange={(e) => patch(sel.id, { borderWidth: parseInt(e.target.value) || 0 })} className={`${inputCls} w-10`} />
              </div>
              <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Radius</span><input type="number" min={0} max={50} value={sel.borderRadius || 0} onChange={(e) => patch(sel.id, { borderRadius: parseInt(e.target.value) || 0 })} className={`${inputCls} w-12`} /></div>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              <div><span className={labelCls}>Script</span>
                <textarea value={sel.text || ""} onChange={(e) => patch(sel.id, { text: e.target.value })}
                  className={`${inputCls} mt-0.5 resize-none`} rows={5} placeholder="Enter your script text..." />
              </div>
              <div className="flex gap-1">
                <select value={sel.fontFamily || "Arial"} onChange={(e) => patch(sel.id, { fontFamily: e.target.value })} className={`${inputCls} flex-1`}>
                  {OVERLAY_FONTS.map(f => <option key={f} value={f}>{f}</option>)}
                </select>
                <input type="number" min={12} max={120} value={sel.fontSize || 24} onChange={(e) => patch(sel.id, { fontSize: parseInt(e.target.value) || 24 })} className={`${inputCls} w-12`} />
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[8px] text-[#6E6E6E]">Direction</span>
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Text</span><input type="color" value={sel.fontColor || "#FFFFFF"} onChange={(e) => patch(sel.id, { fontColor: e.target.value })} className="w-5 h-5 rounded cursor-pointer border-none" /></div>
                <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">BG</span><input type="color" value={sel.backgroundColor === "rgba(0,0,0,0.75)" ? "#000000" : (sel.backgroundColor || "#000000")} onChange={(e) => patch(sel.id, { backgroundColor: e.target.value })} className="w-5 h-5 rounded cursor-pointer border-none" /></div>
              </div>
              <div className="flex items-center gap-1"><span className="text-[8px] text-[#6E6E6E]">Radius</span><input type="number" min={0} max={50} value={sel.borderRadius || 0} onChange={(e) => patch(sel.id, { borderRadius: parseInt(e.target.value) || 0 })} className={`${inputCls} w-12`} /></div>
            </div>
          )}
          {sel.type === "transition" && (
            <div className="space-y-1.5">
              <div>
                ...
                <select value={sel.transitionType || "crossfade"} onChange={(e) => patch(sel.id, { transitionType: e.target.value as any })} className={`${inputCls} mt-0.5`}>
              ...
              {sel.transitionType === "fade-color" && (
                <div className="flex items-center gap-1.5">
                  <span className="text-[8px] text-[#6E6E6E]">Color</span>
              ...
              <div className="text-[8px] text-[#4A4A4A] leading-relaxed">

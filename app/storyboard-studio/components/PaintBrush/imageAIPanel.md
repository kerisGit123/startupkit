...
> **Schema**: See `corePlaning.md` → `storyboardItems.imageGeneration`, `storyboardItems.imageUrl`, `creditUsage`
...
This file does NOT cover project/script management (→ `storyboardplanning.md`), video generation (→ `videoAIPanel.md`), or file storage (→ `filePlaning.md`).
...
export const batchGenerateImages = action({
  ...
  handler: async (ctx, { projectId, style, quality }) => {
    ...
    for (const item of pending) {
      await ctx.runAction(internal.imageGeneration.generateImageForItem, {
        itemId: item._id,
        style,
        quality,
      });
...
export async function POST(request: Request) {
  ...
  if (data.status === 'completed' && data.result?.image_url) {
    // 1. Upload to R2 (→ filePlaning.md handles storage)
    const r2Url = await uploadToR2(data.result.image_url, data.taskId);

    // 2. Update storyboardItem
